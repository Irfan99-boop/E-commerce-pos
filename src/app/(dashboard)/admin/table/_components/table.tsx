...
export default function TableManagement() {
  ...
  console.log("🏗️ TableManagement dirender"); // Log untuk cek
  ...
  const refreshTables = () => {
    console.log("🔄 Invalidating all tables queries...");
    console.log("🔄 ===== REFRESH TABLES DIPANGGIL =====");
    console.log("🔄 Ini harusnya muncul di BROWSER CONSOLE");
    // Ini akan invalidate SEMUA query dengan prefix "tables"
    queryClient.invalidateQueries({
      queryKey: ["tables"],
      exact: false, // false = semua query yang dimulai dengan ["tables"]
    });
    console.log("🔄 ===== INVALIDATE SELESAI =====");
  };
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col lg:flex-row mb-4 gap-2 justify-between w-full">
        ...
        <div className="flex gap-2">
          ...
          <Dialog>
            ...
            <DialogCreateTable refetch={refreshTables} />
          </Dialog>
        </div>
      </div>
      <DataTable
      ...
      />
      <DialogUpdateTable
        key={`delete-${selectedAction?.data?.id || "none"}`} // ✅ Tambah key
